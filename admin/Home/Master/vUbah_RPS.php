...
$sql = "SELECT * FROM tbl_rps WHERE idRPS = $idRPS";
$result = mysqli_query($conn, $sql);

if ($result && mysqli_num_rows($result) > 0) {
    $RPS = mysqli_fetch_assoc($result);
...
<body>
    ...
    <div class="col-sm-9 col-sm-offset-3 col-lg-10 col-lg-offset-2 main">
        ...
        <div class="row">
            <div class="col-lg-12">
                <h1 class="page-header">Update RPS</h1>
            </div>
        </div><!--/.row-->

        <h3 style="margin-bottom: 20px;">Update RPS</h3>
        <form action="../../././../controller/admin/prosesUbah_RPS.php?idRPS=<?= $idRPS; ?>" method="post" enctype="multipart/form-data">
            <?php
            include '../../../database/koneksi.php';
            ...
            if ($result && mysqli_num_rows($result) > 0) {
                ...
                echo '<option value="" selected>-- Pilih ' . $idMK . ' --</option>';

                while ($row = mysqli_fetch_assoc($result)) {
                    echo '<option value="' . $row['idMK'] . '">' . $row['namaMK'] . "(" . $row['idMK'] . ")" . '</option>';
                }
                echo '</select>';
                echo '</div>';
            } else {
