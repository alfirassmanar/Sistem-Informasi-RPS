...
$sql = "SELECT * FROM tbl_pembelajaran WHERE idPembelajaran = $idPembelajaran";
$result = mysqli_query($conn, $sql);

if ($result && mysqli_num_rows($result) > 0) {
    $pembelajaran = mysqli_fetch_assoc($result);

    $idPembelajaran = $pembelajaran['idPembelajaran'];
    $idMahasiswa = $pembelajaran['idMahasiswa'];
    $kodeMK = $pembelajaran['kodeMK'];
} else {
...
<body>
    ...
    <div class="col-sm-9 col-sm-offset-3 col-lg-10 col-lg-offset-2 main">
        ...
        <div class="row">
            <div class="col-lg-12">
                <h1 class="page-header">Ubah Pembelajaran</h1>
            </div>
        </div><!--/.row-->

        <h3 style="margin-bottom: 20px;">Ubah Pembelajaran</h3>
        <form action="../../././../controller/admin/prosesUbah_Pembelajaran.php?idPembelajaran=<?= $idPembelajaran; ?>" method="post">
            <?php
            $sql = "SELECT idMahasiswa, namaMahasiswa FROM tbl_mahasiswa";
            $result = mysqli_query($conn, $sql);

            if ($result && mysqli_num_rows($result) > 0) {
            ?>
                <div class="form-group">
                    <label for="status" style="font-size: 12px;">Nama Mahasiswa :</label>
                    <select class="form-control" id="idMahasiswa" required name="idMahasiswa" style="font-size: 12px;">
                        <option value="" disabled selected>-- Pilih <?= $idMahasiswa ?> --</option>
                        <?php
                        while ($row = mysqli_fetch_assoc($result)) {
                            echo "<option value='" . $row['idMahasiswa'] . "'>" . $row['namaMahasiswa'] . "(" . $row['idMahasiswa'] . ")" . "</option>";
                        }
                        ?>
                    </select>
                    <!-- <input type="hidden" value="<?= $idMahasiswa ?>" name="idMahasiswa"> -->
                </div>
            <?php
            } else {
                echo "<div class='alert alert-warning'>Data mahasiswa tidak tersedia.</div>";
            }
            ?>

            <?php
            $sql = "SELECT kodeMK, namaMK FROM tbl_mk";
            $result = mysqli_query($conn, $sql);

            if ($result && mysqli_num_rows($result) > 0) {
            ?>
                <div class="form-group">
                    <label for="status" style="font-size: 12px;">Kode MataKuliah :</label>
                    <select class="form-control" id="kodeMK" required name="kodeMK" style="font-size: 12px;">
                        <option value="" disabled selected>-- Pilih <?= $kodeMK ?> --</option>
                        <?php
                        while ($row = mysqli_fetch_assoc($result)) {
                            echo "<option value='" . $row['kodeMK'] . "'>" . $row['namaMK'] . "(" . $row['kodeMK'] . ")" . "</option>";
                        }
                        ?>
                    </select>
                    <!-- <input type="hidden" value="<?= $kodeMK ?>" name="kodeMK"> -->
                </div>
            <?php
            } else {
