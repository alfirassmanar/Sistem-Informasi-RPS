...
$sql = "SELECT * FROM tbl_Fakultas WHERE idFakultas = $idFakultas";
$result = mysqli_query($conn, $sql);

if ($result && mysqli_num_rows($result) > 0) {
    $fakultas = mysqli_fetch_assoc($result);

    $idFakultas = $fakultas['idFakultas'];
...
<body>
    ...
    <div class="col-sm-9 col-sm-offset-3 col-lg-10 col-lg-offset-2 main">
        ...
        <div class="row">
            <div class="col-lg-12">
                <h1 class="page-header">Ubah Fakultas</h1>
            </div>
        </div><!--/.row-->

        <h3 style="margin-bottom: 20px;">Ubah Fakultas</h3>
        <form action="../../././../controller/admin/prosesUbah_Fakultas.php?idFakultas=<?= $idFakultas; ?>" method="post">

            <div class="form-group">
                <label for="nama" style="font-size: 12px;">Nama Fakultas :</label>
                <input type="text" class="form-control" id="nama" name="namaFakultas" style="font-size: 12px;" required value="<?= $namaFakultas ?>">
            </div>

            <div class="form-group">
                <label for="status" style="font-size: 12px;">Status :</label>
                <select class="form-control" id="status" name="status" style="font-size: 12px;" required>
